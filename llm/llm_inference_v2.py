...
DRUGBANK_WITH_OPTIONS_PROMPT = (
    "Instructions: Answer the following questions about drug interactions using the given knowledge graph information (if available), essential parts of the drug definitions.\n"
...
DEFAULT_INSTRUCT_TEMPLATE = (
    "Instructions: Answer the following questions about drug interactions.\n"
    "Context: You are a pharmacodynamics expert, base your answer on evidence of known interaction mechanisms, pharmacological effects, or similarities to related compounds if applicable.\n" 
    "Focus exclusively on the primary interaction type between the specified drugs and conditions.\n"
  )


SYSTEM_INSTRUCTION_PROMPT = {
    "drugbank_with_options": lambda has_context:
        DRUGBANK_WITH_OPTIONS_PROMPT if has_context else DEFAULT_INSTRUCT_TEMPLATE,

    "drugbank_with_bullets": lambda has_context:
        DRUGBANK_WITH_OPTIONS_PROMPT if has_context else DEFAULT_INSTRUCT_TEMPLATE,

    "drugbank_open": lambda has_context: 
        DRUGBANK_OPEN_PROMPT if has_context else DEFAULT_INSTRUCT_TEMPLATE,

    "ddinter_common": lambda has_context: 
        DDINTER_PROMPT if has_context else DEFAULT_INSTRUCT_TEMPLATE,

    "pharmacotherapydb": lambda has_context: 
        PHARMACOTHERAPYDB_KG_PROMPT if has_context else DEFAULT_INSTRUCT_TEMPLATE,
}
...
def drugbank_chat_template(dataset, tokenizer, args):
    ...
    def apply_template(example):
        has_context = args.use_kg or args.use_drug_descriptions
        if args.use_options:
        ...
        if args.use_options and args.option_style == "numbered":
            system_prompt = SYSTEM_INSTRUCTION_PROMPT["drugbank_with_options"](has_context)
        elif args.use_options and args.option_style == "bulleted":
            system_prompt = SYSTEM_INSTRUCTION_PROMPT["drugbank_with_bullets"](has_context)
        else:
            system_prompt = SYSTEM_INSTRUCTION_PROMPT["drugbank_open"](has_context)
        ...
        Answer_template = ("Your answer must be concise and formatted as follows: ##Answer: <DrugX the specific effect or mechanism of interaction DrugY>\n") #Drugbanks
        user_content += Answer_template
...
def ddinter_chat_template(dataset, tokenizer, args):
    def apply_template(example):
        ...
        system_prompt = SYSTEM_INSTRUCTION_PROMPT["ddinter_common"](has_context)
        ...
        Answer_template = ("Your answer must be concise and formatted as follows: ##Answer: <the specific level of severity>\n") #DDinter
...
def pharmacotherapydb_chat_template(dataset, tokenizer, args):
    ...
    def apply_template(example):
        ...
        system_prompt = SYSTEM_INSTRUCTION_PROMPT["pharmacotherapydb"](has_context)
        ...
        Answer_template = ("Your answer must be concise and formatted as follows: ##Answer: <Indication>.\n") #DRD

        user_prompt += Answer_template
